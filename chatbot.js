...
const bot = options_reply([
	...
	[/^(ajuda|tutorial|\?)[.]?/i, (match) => thread("Ajuda", [
		"É bom saber algumas coisas sobre mim para ter melhor interação.\nPara prosseguir mensagens voce pode escrever coisas como 'ok', 'blz', 'prosseguir', 'continuar' etc.",
		"Ótimo, as vezes voce terá opções de letra para escolher como por exemplo:\na) Continuar o tutorial\nb) Sair do tutorial\n\nPara escolher apenas envie a letra escolhida."
	], ajuda_options)],
])

const ajuda_options = options_reply([
	[/^a$/i, (match) => "Continuando, há momentos que voce poderá fazer perguntas como 'O que é ***' ou 'Para que serve ***', sinta-se livre para fazer essas perguntas. Escreva 'encerrar' para sair do tutorial."],
...
const compiler = options_reply([
	...
	[/^b$/i, (match) => thread("Optimizações", [
		...
		"A maneira que o computador funciona nem sempre é do mesmo que nós esperamos. Por exemplo em vez de dividir um numero inteiro por 4 é mais rapido mover o numero 2 bits para a esquerda.",
		"Compiladores fazem essas optimizações para que o codigo aproveite mais da capacidade do computador, sendo assim mais eficiente.",
...
const lex_parse = options_reply([
	[/o que (é|e) (um |o )?parser/i, (match) => thread("O que é parser", [
		...
		`Voce pode ver um parser de expressões matematicas formando uma AST digitando 'parse <expressão>'\nEx.:\nparse 23 + 32 * 7`
	], lex_parse)],
	[/o que (é|e) (um |o )?lexer/i, (match) => thread("O que é lexer", [
		"O lexer é responsavel por transformar o texto em unidades chamadas tokens que são como \"palavras\" numa linguagem natural.",
		"O lexer é usado em conjunto com um parser.",
		`Voce pode ver o trabalho de um lexer de expressões matematicas digitando 'lexer <expressão>'\nEx.:\nlexer 23 + 32 * 7`
	], lex_parse)],
	[/pa?ra que (serve|usa) (um |o )?lexer/i, (match) => thread("Para que serve o lexer", [
		"Lexers são usados em conjunto com parsers, eles estão envolvidos em qualquer coisa que involve transformar texto em uma estrutura de dados.",
		"Exemplos disso são: ler arquivos JSON, compilar/interpretar linguagens de programção, syntax highlight em editores de texto.",
		"É bem provavel que voce não vai precisar implementar um lexer ou parser, mas é bom saber que eles sempre são usados quando se programa.",
	], lex_parse)],
	[/pa?ra que (serve|usa) (um |o )?parser/i, (match) => thread("Para que serve o parser", [
		"Parsers são usados em conjunto com lexers, eles estão envolvidos em qualquer coisa que involve transformar texto em uma estrutura de dados.",
		"Exemplos disso são: ler arquivos JSON, compilar/interpretar linguagens de programção, syntax highlight em editores de texto.",
		"É bem provavel que voce não vai precisar implementar um lexer ou parser, mas é bom saber que eles sempre são usados quando se programa.",
	], lex_parse)],
...
const optimize = options_reply([
	[/^a$/i, (match) => thread("Loop Optimization", [
		...
		"Por isso foram criados varios tipos de optimzações envolvendo eles, essas optimizações consistem principalmente de transformar coisas que parecem constantes em constantes e prever repetições antes de acontencerem.\n\nVocê pode ver outra opção agora.",
	], optimize)],
	[/^b$/i, (match) => thread("Register Alocation", [
		"Alocação de registradores acontece na hora de transformar codigo em Assembly.",
		"A alocação de registradores busca usar a menor quantidade de registradores da maquina, porque o numero de registradores é muitas vezes limitado e eles são usados para multiplos propósitos.",
		"Fazer isso tambem requer saber usar a memoria de maneira eficiente quando há falta de registradores.\n\nVocê pode ver outra opção agora."
	], optimize)],
	[/^c$/i, (match) => thread("Instruction Scheduling", [
		"Instruction Scheduling é uma optimização que veio com os processadores modernos, esses processadores podem ler um intrução a frente da que está executando e executar coisas em menos ciclos.",
		"Agendamento de instruções são tecnicas que tentam aproveitar disso ao máximo.",
		"Isso é feito alterando a ordem de instruções do processador para evitar ciclos parados sem mudar o funcionamento do codigo.\n\nVocê pode ver outra opção agora."
	], optimize)],
	...
	[/^e$/i, (match) => thread("Bounds Check Elimination", [
		...
		"Isso é possível quando o tamanho da lista ou array é conhecido no tempo de compilação e assim é possivel saber se acessos a ele são válidos ou não.\\Você pode ver outra opção agora."
	], optimize)],
	[/^voltar/i, (match) => go_back()],
])
...
const paradigm = options_reply([
	[/^a$/i, (match) => thread("Imperativo", [
		"Em proramação imperativa voce escreve o programa como uma série de ordens para o computador, há dois paradigmas principais derivados dele.",
		"Procedural:\n\nO paradigma procedural deriva da programação estruturada, nele as intruções ficam em procedimentos",
		"Com isso codigo pode ser reutilizado, fica mais organizado e modular.",
	...
	[/^b$/i, (match) => thread("Declarativo", [
		"Em programação declarativa voce descreve o que o programa faz, há dois paradigmas principais derivados dele."
		"Funcional:\n\nEm programação funcional as funções são elementos de primeira classe e todas retornam um valor assim como a definição matemática, ela tambem valoriza a imutabilidade."
		"Esse paradigma divide o programa em varias funções pequenas tornando-o muito modular e poder usar funções como argumento garante muita flexibilidade e composição. Por conta da imutabilidade tambem é comummente usado junto a programação concorrente."
	...
	[/^c$/i, (match) => "O paradigma de programação reflexivo permite que a linguagem trate de coisas dela mesmo enquanto o codigo executa.",
	[/^d$/i, (match) => "Em programação concorrente o código não é executado em sequencia, então pode ter duas funções executando ao mesmo tempo ou que aparentam estar, é util para tarefas que podem ser realizadas em paralelo, como a verificação de dados de varios usuários num banco de dados.",
	[/^voltar/i, (match) => go_back()],
])
